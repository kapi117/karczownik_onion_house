...
HAL_StatusTypeDef get_red(ColorSensor *color_sensor, uint16_t* value){
	HAL_StatusTypeDef status = readRegword(TCS34725_RDATAL, value);
	return status;
}

HAL_StatusTypeDef get_green(ColorSensor *color_sensor, uint16_t* value){
	HAL_StatusTypeDef status = readRegword(TCS34725_GDATAL, value);
	return status;
}

HAL_StatusTypeDef get_blue(ColorSensor *color_sensor, uint16_t* value){
	HAL_StatusTypeDef status = readRegword(TCS34725_BDATAL, value);
	return status;
}

HAL_StatusTypeDef get_light(ColorSensor *color_sensor, uint16_t* value){
	HAL_StatusTypeDef status = readRegword(TCS34725_CDATAL, value);
	return status;
}
